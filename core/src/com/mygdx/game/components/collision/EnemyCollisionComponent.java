...
public class EnemyCollisionComponent extends Component implements Collidable {
    ...
    public void handleCollision(Engine engine, Entity collider, Entity collidee) {
        ...
        if (type == PhysicsManager.COL_FRIENDLY_BULLET) {
            ...
            PositionComponent posCom = new PositionComponent(sprite.getX() + sprite.getWidth() / 2 - bFontCom.bFont.getBounds(bFontCom.msg).width / 2, sprite.getY() + sprite.getHeight() / 2 + bFontCom.bFont.getBounds(bFontCom.msg).height / 2, RenderPriority.HIGH);
